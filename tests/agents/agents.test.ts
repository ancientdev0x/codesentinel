...
describe('reviewer agent', () => {
  ...
  const agentsMd =
    '# Project rules\nAlways write tests for new behaviour. CodeSentinel_MARKER_42'

  beforeAll(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'CodeSentinel-reviewer-'))
  ...
  it('initializes to a config with model, suggest_change tool, and injected AGENTS.md', async () => {
    ...
    expect(cfg.instructions as string).toContain('CodeSentinel_MARKER_42')
