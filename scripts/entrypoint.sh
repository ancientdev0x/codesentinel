#!/usr/bin/env bash
# Run the QA workflow via the PREBUILT server (dist/server.mjs) using the CodeSentinel
...
WORKDIR="${CodeSentinel_QA_WORKSPACE:-${PWD:-/app/.qa-run}}"
mkdir -p "$WORKDIR"
cd "$WORKDIR"
exec node /app/bin/CodeSentinel.mjs qa
