...
const GITHUB_REPO = 'https://github.com/ancientdev0x/CodeSentinel'
...
const faqs: Faq[] = [
  {
    q: 'What is CodeSentinel?',
    a: (
      <>
        CodeSentinel is an extendable, open-source AI code review agent. It reads your diff,
  ...
  },
  {
    ...
    a: (
      <>
        Run <code className="font-mono text-foreground">npx CodeSentinel init</code> to
        scaffold a GitHub Action that reviews every pull request, then add your provider
        API key as a repo secret. You can also run{' '}
        <code className="font-mono text-foreground">npx CodeSentinel review</code> locally to
  ...
  },
  {
    ...
    a: (
      <>
        CodeSentinel is provider-agnostic. Anthropic, OpenAI, OpenRouter, and Cloudflare
        Workers AI are supported out of the box — just set the matching API key.
      </>
    ),
  },
  {
    q: 'Can I extend CodeSentinel with my own tools?',
    a: (
      <>
        Yes. CodeSentinel acts as a Model Context Protocol (MCP) client, so you can connect
