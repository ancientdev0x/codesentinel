...
CodeSentinel runs its review agent on flue (which drives the agent loop on "pi"). For focused, token-heavy investigation the reviewer doesn't spawn a bespoke sub-agent — it uses flue's built-in **`task`** tool, optionally targeting a named **subagent profile** declared on the agent.
...
- CodeSentinel's custom `suggest_change` tool (inline review comments) belongs to the top-level reviewer; the PR summary is the reviewer's final message.
- MCP tools are exposed as `mcp__<name>__<tool>` and are available subject to your `MCP_SERVERS` / `CodeSentinel_MCP_SERVERS` configuration.
