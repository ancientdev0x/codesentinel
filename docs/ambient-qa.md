# CodeSentinel QA — Ambient Autonomous QA Agent

> **Status:** Planned. This document is both the **plan** and the **running log** for building
> **`CodeSentinel qa`** — a second prebuilt flue agent, sibling to `CodeSentinel review`, that autonomously
...
Turn CodeSentinel into an **ambient QA agent** that does for *end-to-end QA* what `CodeSentinel review` does for
diffs. It:
...
- Reuses every piece of CodeSentinel's existing plumbing (`resolveReviewConfig`, the octokit `Reporter`,
  `connectMcpServers`, `telemetry`, `bin/CodeSentinel.mjs`, the composite `action.yml` shape).
...
This is a large additive feature. It introduces no breaking changes to `CodeSentinel review`; the two
agents live side-by-side and migrate together (see §13, beta.3).

## 2. Background: where this builds from

- **flue / pi** — same runtime as `CodeSentinel review`. "Agent = Model + Harness." We compose with
...
**`CodeSentinel qa` is a v0 MONOLITH ON ONE GITHUB RUNNER (LOCKED).** The flue agent loop, the built-in
...
GitHub Actions runs `CodeSentinel qa` **once per cron tick / dispatch**. flue's **workflow + `flue run`**
...
bin/CodeSentinel.mjs                  (edit: add `qa` + `qa init` subcommands)
qa/action.yml                  ✚  composite action (clone of action.yml; runs `flue run qa`)
scaffold/
  CodeSentinel-qa.yml               ✚  template scaffolded by `CodeSentinel qa init`
...
`isoWeekBranch()` = `CodeSentinel-qa/$(date -u +%G-W%V)`; `materializeSkill`/`writeCatalog`/`listArtifacts`/
...
export const createOpenPrTool = (cfg: QaConfig) =>
  defineTool({
    ...
    parameters: v.object({
      ...
      branch: v.optional(v.string()),  // default CodeSentinel-qa/<iso-week>
    }),
...
export async function openOrUpdatePr(cfg: QaConfig, a: OpenPrArgs): Promise<OpenPrResult> {
  ...
  const branch = a.branch ?? isoWeekBranch()                 // CodeSentinel-qa/2026-W26
...
- **`src/qa/config.ts`** — `QaPayload` adds `target?` (URL/path under test), `scope?`, `branch?`,
  `chromeBin?`. `resolveQaConfig` reuses the review env-fallback pattern (`CodeSentinel_QA_TARGET`,
  `CodeSentinel_QA_SCOPE`, `CodeSentinel_QA_BRANCH`, `CHROME_BIN`) and reuses `resolveReviewConfig`'s
  GitHub-target resolution for `owner/repo/token`.
- **`bin/CodeSentinel.mjs`** — add a `qa` namespace next to `review`/`init`/`configure`. `CodeSentinel qa` boots
  `dist/server.mjs` and POSTs `/workflows/qa` (clone of the `review` boot path). `CodeSentinel qa init`
  scaffolds `.github/workflows/CodeSentinel-qa.yml` + a starter `playwright.config.ts`, with the same
...
**Target repo already has a `playwright.config.ts`?** Do **not** overwrite it. `CodeSentinel qa init` writes the
...
shipped by `CodeSentinel qa init` in v0 therefore lists only `ubuntu-latest` in the verify job — it is expanded to
...
1. **Deterministic week-stamped branch:** `CodeSentinel-qa/$(date -u +%G-W%V)` → `CodeSentinel-qa/2026-W26`. Keyed
...
2. **Existing-PR guard inside `open_pull_request`:** `octokit.rest.pulls.list({ state:'open',
   head:'owner:CodeSentinel-qa/<week>' })` — if one is open, push onto it (update) instead of opening a new
   ...
   `concurrency.group: CodeSentinel-qa` prevents overlapping runs.

## 9. GitHub Actions operational layer

**The scaffolded target-repo workflow** (`CodeSentinel qa init` writes `.github/workflows/CodeSentinel-qa.yml`):

```yaml
name: CodeSentinel QA 🧪
on:
...
concurrency: { group: CodeSentinel-qa, cancel-in-progress: false }
jobs:
  author:                                      # LINUX ONLY · has agent + key · writes specs · opens PR
    ...
    steps:
      ...
      - id: qa                                  # the composite parses the agent's final JSON and writes
        uses: ancientdev0x/CodeSentinel/qa@v0          #   branch/changed/prUrl to $GITHUB_OUTPUT (see seam below)
        with:
  ...
  verify:                                       # NO agent · NO key · plain `playwright test`
    ...
    steps:
      ...
      - run: npx playwright test
        env: { E2E_BASE_URL: ${{ inputs.target || 'https://CodeSentinel.dev' }} }
      - if: ${{ !cancelled() }}
...
- **Cron + dispatch** both present. On demand:
  `gh workflow run CodeSentinel-qa.yml -f target=https://staging.example.com -f scope="checkout + login"`.
- For any **comment-triggered** variant, reuse the **`author_association` gate** from
  `.github/workflows/CodeSentinel-mention.yml` (the "pwn request" mitigation accepted during the migration).
- **`CodeSentinel qa init` "next steps"** must flag the org/repo setting **"Allow GitHub Actions to create and
...
`CodeSentinel-qa.yml`, authenticated by a GitHub App installation token.** Each target then runs the suite
  ...
  dispatch:
    ...
    steps:
      ...
      - env: { GH_TOKEN: ${{ steps.app.outputs.token }} }
        run: gh workflow run CodeSentinel-qa.yml --repo "${{ matrix.repo }}" -f target="https://${{ matrix.repo }}.example.dev"
```
...
there. **Optional central-upgrade implementation:** make the scaffolded `CodeSentinel-qa.yml` a thin
`uses: ancientdev0x/CodeSentinel/.github/workflows/qa-reusable.yml@v0` caller (a `workflow_call` reusable
workflow), so the pipeline version-bumps centrally while still composing with the fan-out.

> **Setup guide:** see [docs/cross-repo-qa.md](./cross-repo-qa.md) for the step-by-step GitHub App + `CodeSentinel qa fanout-init` runbook, the reusable-workflow caller, and the escape hatch.
...
```dockerfile
# Dockerfile — the CodeSentinel-qa monolith
...
-e ANTHROPIC_API_KEY -e GITHUB_TOKEN CodeSentinel-qa <args>` locally **==** the CI step, byte-for-byte.
...
- **Linux author leg → runs inside the image** (`jobs.author.container.image:
  ghcr.io/ancientdev0x/CodeSentinel-qa:<sha>`). The image *is* the contract; CI runs exactly your Dockerfile.
...
**Key existing files this builds on (absolute):** `/Users/matt/Documents/Github/CodeSentinel/src/agents/reviewer.ts`,
`/Users/matt/Documents/Github/CodeSentinel/src/workflows/review.ts`,
`/Users/matt/Documents/Github/CodeSentinel/src/review/config.ts`,
`/Users/matt/Documents/Github/CodeSentinel/src/github/reporter.ts`,
`/Users/matt/Documents/Github/CodeSentinel/src/tools/suggest-change.ts`,
`/Users/matt/Documents/Github/CodeSentinel/src/mcp/connect.ts`,
`/Users/matt/Documents/Github/CodeSentinel/src/common/telemetry.ts`,
`/Users/matt/Documents/Github/CodeSentinel/action.yml`,
`/Users/matt/Documents/Github/CodeSentinel/.github/workflows/CodeSentinel-mention.yml`,
`/Users/matt/Documents/Github/CodeSentinel/bin/CodeSentinel.mjs`,
`/Users/matt/Documents/Github/CodeSentinel/package.json`,
`/Users/matt/Documents/Github/CodeSentinel/flue.config.ts`. Vendored skill to copy into
...
- **v0b — operational shell** (`6b87e9e` PR machinery + seams; `00cb7cd` packaging + ops):
  ...
  `bin/CodeSentinel.mjs` `qa` / `qa init`. `files` ships `src/skills`. Tests: pr-policy + openOrUpdatePr
  (open/empty/update/local).

**Verified:** all gates green (91 tests: 89 pass + 2 opt-in CDP skipped); `flue build` discovers the new
agent + workflow; patched CDP client drives real Chrome; `CodeSentinel qa init` scaffolds correctly.
...
- **Docker fix (the monolith now runs):** the entrypoint runs the **prebuilt `dist/server.mjs`** (via
  `CodeSentinel qa`), NOT `npx flue run` — `@flue/cli` eagerly imports miniflare→workerd, whose platform binary
...
- **Environment caveat (not a bug):** behind a corporate **TLS-inspecting proxy**, external HTTPS targets
  (e.g. CodeSentinel.dev) fail from the container with `SELF_SIGNED_CERT_IN_CHAIN` (OpenAI is allowlisted, so the
...
- **Adversarial review workflow** (4 dims → verify → synthesis) ran on the pivot: 21 candidates → 17
  ...
  (`vars.CodeSentinel_QA_TARGET`) + author→verify `base_url`; **H2** Docker entrypoint honors `-w` (output lands
...
- **LIVE, in the Docker monolith, against the real https://CodeSentinel.dev (external HTTPS through a corporate
  TLS-inspecting proxy):** \`openai/gpt-5.5\` **PASSED** — catalogued 3 flows, drove the live site over the
  cert-tolerant CDP path, wrote a dependency-free \`e2e/tests/CodeSentinel-landing-hero-nav-faq.cdp.mjs\` that
...
- **kimi-k2.7-code on Workers AI:** **PASSED** a minimal-scope run on the new format end-to-end (live
  CodeSentinel.dev → \`e2e/tests/hero-loads.cdp.mjs\` green + screencast). Longer multi-call runs intermittently
  ...
  CodeSentinel-qa defect. A transient model-call retry belongs in the flue/pi provider layer.
...
### 2026-06-24 — Architecture: CodeSentinel is self-contained; the TARGET is what varies

**LOCKED clarification (maintainer):** CodeSentinel runs in **its own fixed environment** (node + Chrome + ffmpeg,
Linux/macOS — bash-land). The **target runs in the target's environment**; the two talk **over a wire**.
CodeSentinel never becomes the target's OS/runtime.
- **Web app:** the app runs wherever the dev runs it (any OS) and exposes a **URL**; CodeSentinel points its
  browser at that URL. The only per-target knob on CodeSentinel's side is **the browser it hands the agent**
  (size/device) → \`E2E_VIEWPORT\` (\`1280x900\`|\`375x812@2\`|mobile|tablet|desktop), wired through
  cdp-client + \`CodeSentinel_QA_VIEWPORT\` + the qa action \`VIEWPORT\` input.
- **Non-web (e.g. a Rust lib):** the lib runs on its own matrix/VM (its env); CodeSentinel interacts via a
  different dev tool (a terminal) — a later phase. **Cross-OS is therefore a property of where the TARGET
  runs (its CI matrix / a provisioned VM via the Compute/target seam), NOT of CodeSentinel.** So we **dropped the
  win32 Chrome path** — CodeSentinel's browser only ever runs in CodeSentinel's env (the `flue` bash tool itself uses
...
- **Container vs npm (decided): npm-package + GitHub Action is the PRIMARY, lightweight path** — the runner
  *is* CodeSentinel's fixed env (ubuntu ships Chrome; ffmpeg is one apt step; node always; no 187 MB Playwright
...
- **Env-awareness:** \`buildQaInstructions\` injects \`process.platform\` so the agent knows its own shell
  (setsid on linux, nohup+disown on macOS) — about *CodeSentinel's* env, not the target's.

### 2026-06-24 — Phase 1 fan-out: mechanism proven live

Ran a multi-flow fan-out in the docker monolith against the real CodeSentinel.dev at \`mobile\` viewport
...
- **Net:** the fan-out mechanism + viewport knob are proven; the long-run completion limit is the corporate
  proxy (env), not CodeSentinel. On GitHub-hosted runners (no such proxy) long fan-out runs complete.
...
- **Cross-OS** \`CodeSentinel qa init --cross-os\` (3-OS verify matrix).
- **Cross-repo dispatch:** \`CodeSentinel qa fanout-init <repos>\` scaffolds a control-repo fan-out that mints a
  GitHub App installation token (\`actions/create-github-app-token@v1\`, actions:write) and \`gh workflow run\`s
  each target's own CodeSentinel-qa.yml (each runs under its OWN GITHUB_TOKEN — default token can't cross repos);
  \`qa-reusable.yml\` (workflow_call) + \`docs/cross-repo-qa.md\`.
- **CLI/terminal target (kind=cli):** the NON-WEB half. \`CodeSentinel_QA_KIND=cli\` → the agent runs the target's
...
- **#6 — the scaffolded Actions YAML is real YAML.** `bin/CodeSentinel.mjs` embedded ~234 lines of workflow YAML
  ...
  the only remaining escapes are the 2 in the dynamic cross-OS matrix fragment. `bin/CodeSentinel.mjs` drops to
...
in `healer.ts`), disconnected from the config. So `CodeSentinel_QA_MODEL` moved the lead but NOT the drivers.
...
- review + `/CodeSentinel` mention → `CodeSentinel_MODEL` → `DEFAULT_MODEL` (sonnet).
- qa lead + healer → `CodeSentinel_QA_MODEL` → `CodeSentinel_MODEL` → `DEFAULT_QA_LEAD_MODEL` (opus).
- qa drivers → `CodeSentinel_QA_DRIVER_MODEL` → `CodeSentinel_QA_MODEL` → `CodeSentinel_MODEL` → `DEFAULT_MODEL` (sonnet).

Net effect: **`CodeSentinel_MODEL` alone configures the whole system**, and each role has one documented override
...
New `tests/common/models.test.ts` pins the precedence; the earlier failing scenario (`CodeSentinel_MODEL=cloudflare`
