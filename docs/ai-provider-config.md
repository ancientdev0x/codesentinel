...
CodeSentinel selects a model with a single `provider/model` string. The provider prefix
...
- **Environment variable** — `CodeSentinel_MODEL` (used by the Action, also works locally):

  ```bash
  CodeSentinel_MODEL=openai/gpt-4.1-mini flue run review --target node
  ```
...
Set it with the `THINKING_LEVEL` Action input, the `CodeSentinel_THINKING_LEVEL` env var,
