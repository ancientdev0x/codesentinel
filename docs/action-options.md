...
CodeSentinel ships as a composite GitHub Action that runs the prebuilt code-review agent on
your pull requests. It sets up Node 22, installs CodeSentinel, and runs `npx flue run review`.
...
- check out with **`fetch-depth: 0`** so CodeSentinel can diff the full PR range, and
...
jobs:
  review:
    if: ${{ github.event.requested_reviewer.login == 'your-CodeSentinel-bot' }}
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v4
        with:
