...
CodeSentinel 🚢 is a prebuilt code-review agent built on [flue](https://github.com/withastro/flue). It runs the agent loop on "pi" and reviews your code through a one-shot review workflow — either as a GitHub Action on pull requests, or locally against your staged changes.
...
Add a workflow that runs CodeSentinel on every pull request. The job must check out with `fetch-depth: 0` (so the full diff is available) and grant `pull-requests: write` and `contents: read` permissions.

Create `.github/workflows/CodeSentinel.yml`:
...
CodeSentinel also runs locally with no server, reviewing your **staged** changes (`git diff --cached`).
...
This is an alias for `flue run review --target node` with the local platform. In local mode CodeSentinel reviews your staged diff and writes the results to `.CodeSentinel/review/local_*.md`.
...
CodeSentinel can build itself into a publishable, runnable server:
