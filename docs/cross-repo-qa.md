# Running CodeSentinel QA across many repos (cross-repo fan-out)

Run [CodeSentinel QA](./ambient-qa.md) across many repositories you own from a single
...
The control repo only **dispatches** each target repo's *own* `CodeSentinel-qa.yml`. The
...
─────────────                               ────────────────
CodeSentinel-qa-fanout.yml                       CodeSentinel-qa.yml
...
```sh
CodeSentinel qa fanout-init me/app-one,me/app-two me/marketing-site
```

This writes `.github/workflows/CodeSentinel-qa-fanout.yml` with those repos in the dispatch
...
```sh
CodeSentinel qa init   # writes .github/workflows/CodeSentinel-qa.yml; commit + push to the default branch
```
...
The dispatch targets the **default branch**, so `CodeSentinel-qa.yml` must be committed there.
...
- **On demand:**
  ```sh
  gh workflow run CodeSentinel-qa-fanout.yml
...
CodeSentinel ships a reusable `workflow_call` workflow at
[`.github/workflows/qa-reusable.yml`](../.github/workflows/qa-reusable.yml). A target repo
can replace its inlined `CodeSentinel-qa.yml` with a thin caller, which lets you version-bump the
...
jobs:
  qa:
    uses: ancientdev0x/CodeSentinel/.github/workflows/qa-reusable.yml@v0
    with:
...
Prefer the **inlined** workflow (the `CodeSentinel qa init` default) when you want a
...
For repos you **cannot** scaffold (can't add `CodeSentinel-qa.yml`, or org policy forbids Actions
...
- **Dispatch 404** — `CodeSentinel-qa.yml` isn't on the target's default branch yet.
