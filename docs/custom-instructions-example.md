...
3. The `CodeSentinel_CUSTOM_INSTRUCTIONS` environment variable
...
```bash
CodeSentinel_CUSTOM_INSTRUCTIONS="Focus on performance: memory usage, allocations, and algorithm complexity." \
...
`CLAUDE.md`, which CodeSentinel injects into the agent's instructions automatically.
