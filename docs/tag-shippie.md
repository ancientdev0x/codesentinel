# Run CodeSentinel on demand (`/CodeSentinel` on a PR)

Comment `/CodeSentinel` on a pull request to run CodeSentinel on demand. There are two ways to wire it up:
...
> We use `/CodeSentinel` (a command), not `@CodeSentinel` (which would imply a real GitHub user account).

---

## Actions on comment (no server) — recommended

Add a workflow that runs when a PR comment contains `/CodeSentinel`. It resolves the PR's refs and runs the
CodeSentinel action — no hosting, just Actions minutes.

```yaml
# .github/workflows/CodeSentinel-mention.yml
  ...
  CodeSentinel:
    if: >-
      ${{ github.event.issue.pull_request &&
          ...
          contains(github.event.comment.body, '/CodeSentinel') &&
...
Then comment `/CodeSentinel review` on any pull request. This runs the same review as the per-PR action
(inline comments + a summary). This repo dogfoods it in `.github/workflows/CodeSentinel-mention.yml`.
...
> `/CodeSentinel`. CodeQL still flags this as an "untrusted checkout" — it's an accepted, gate-mitigated risk
...
> The Actions-on-comment path runs a **review**. For free-form questions (`/CodeSentinel does X handle Y?`),
...
CodeSentinel also ships a GitHub **channel** (`src/channels/github.ts`, built on
[`@flue/github`](https://github.com/withastro/flue)). Deployed as a server it serves a webhook at
`POST https://<your-host>/channels/github/webhook`; a `/CodeSentinel` comment is verified, dispatched to the
...
CodeSentinel deploys anywhere Flue does (Node, Cloudflare, Render, Fly…). The simplest is Node:
...
| `GITHUB_TOKEN` | **Required.** Authenticates CodeSentinel's reply comments + diff fetches. |
| `CodeSentinel_MODEL` | Model specifier (default `anthropic/claude-sonnet-4-6`). |
...
Then comment `/CodeSentinel review` (or ask a question) on any issue or pull request. CodeSentinel ignores comments
