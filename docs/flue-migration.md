# CodeSentinel → Flue Migration Log

> **Status:** In progress. This document is both the **plan** and the **running log** for
> refactoring CodeSentinel from a bespoke Vercel-AI-SDK agent into a **prebuilt code-review agent
...
Turn CodeSentinel into a **prebuilt flue agent**: a code-review agent that ships as a flue project so it
...
- **"pi"** — `@earendil-works/pi-agent-core` / `@earendil-works/pi-ai` (Mario Zechner's pi). This is
  ...
  pi tools" = use flue's built-in `read/write/edit/bash/grep/glob/task` tools rather than CodeSentinel's
...
- **`AGENTS.md`** is discovered by flue (system prompt / `task` tool per-directory). **`CLAUDE.md` and `.cursor/rules` are NOT supported by flue** — CodeSentinel must read those itself and inject them.
- Structured results: `session.prompt(text, { result: v.object({...}) })` → validated `response.data`.

## 3. Architecture decision

**CodeSentinel becomes a flue project whose primary entry is a one-shot review workflow.**

```
CodeSentinel/  (the flue project AND the published GitHub Action)
...
GitHub Actions runs CodeSentinel **once per PR**. flue's **workflow + `flue run`** model is the exact fit:
...
`@CodeSentinel review` comments), not the CI path. Both can coexist later.

### Where the diff comes from

Keep CodeSentinel's deterministic diff computation (`src/common/git/*`) — it drives file filtering and the
...
| CodeSentinel module | Fate | Flue equivalent / notes |
| --- | --- | --- |
| `src/index.ts` (yargs dispatch) | **Remove** | `flue run review` is the entry; keep an optional thin `CodeSentinel` bin only if we still want a local CLI. |
...
| `src/configure/*` + `templates/*` | **Replace** | Drop multi-platform templates; ship one `action.yml` + a documented workflow snippet. Optional `CodeSentinel init` later. |
...
2. **MCP stdio gap** — flue's `connectMcpServer` is remote-only. CodeSentinel's old `.mcp.json` allowed
...
4. **Inline review comments** — Octokit `pulls.createReviewComment` (path/line) is our code, not a
   flue affordance. Keep CodeSentinel's existing single/multi-line logic.
...
- **Research complete.** Pulled 66 flue source/doc files to `/tmp/flue-ref`; ran a 6-agent
  ...
  CodeSentinel module to its flue fate (section 4). Decided on the **workflow + `flue run`** architecture
  (section 3).
...
- `src/github/reporter.ts` — tiny `github | local` reporter (replaces the whole `platform/` abstraction): inline review comments + summary upsert via `octokit`, or append to `.CodeSentinel/review/*.md` locally.
...
- **Rewrote `action.yml`** → flue-native composite action: `actions/setup-node@v4` (Node 22) →
  ...
  All config flows via env (`CodeSentinel_MODEL`, `CodeSentinel_*`, `BASE_SHA`/`HEAD_SHA`, `CodeSentinel_PR_NUMBER`,
  `CodeSentinel_MCP_SERVERS`, provider keys, `GITHUB_TOKEN`) — no `jq`/payload juggling. `config.ts` extended
...
- **Tests (flue-aware):** refactored `diff.ts` to export `parseDiff(rawDiff, workspace)` and added
  ...
  resolution, `CodeSentinel_IGNORE` parsing, MCP env parsing bare/wrapper/invalid). **`bun test src/review` →
...
### 2026-06-17 — Re-add telemetry + `npx CodeSentinel` CLI bin (maintainer request)

- **Telemetry (re-added):** `src/common/telemetry.ts` — fire-and-forget anonymous `review_started` POST to
  `https://telemetry.CodeSentinel.dev/events` (anonymized repo id = sha256 of owner/repo or workspace, run id,
  platform, model, host info — no code/file contents). Wired into the workflow after the changed-files check.
  Opt-out via `CodeSentinel_TELEMETRY=false` (env), the `TELEMETRY` Action input, or `payload.telemetry: false`;
  on by default. No new deps (`crypto.randomUUID`, global `fetch` with a 3s abort).
- **CLI bin (`npx CodeSentinel`):** `bin/CodeSentinel.mjs` + `package.json` `bin`. `CodeSentinel review` boots the prebuilt
...
- Verified: biome/tsc/tests/build all green; `CodeSentinel --help` works; `node bin/CodeSentinel.mjs review` runs an
...
- **Maintainer decisions resolved** (done above): telemetry re-added (opt-out), `npx CodeSentinel` CLI bin added.
...
**CodeSentinel reviewed its own PR via the GitHub Action and posted a summary comment** ("## General Summary
...
### 2026-06-17 — Live `@CodeSentinel` channel + dogfood model `gpt-5.4-nano` (maintainer request)

- **GitHub channel (webhook / server mode):** added `src/channels/github.ts` (`@flue/github`
  `createGitHubChannel`) + `src/agents/mention.ts`. Commenting `@CodeSentinel …` on an issue/PR (verified
  ...
  `docs/tag-CodeSentinel.md` (+ README + `.env.example`).
...
### 2026-06-17 — `/CodeSentinel` on-demand trigger (Actions, no server) + rename `@` → `/`

- Trigger is now **`/CodeSentinel`** (a command), not `@CodeSentinel` (which would imply a real GitHub user account).
  Updated the channel (`src/channels/github.ts`), the `mention` agent, and docs.
- Added **Actions-on-comment** mode (no server): `.github/workflows/CodeSentinel-mention.yml` runs the action when
  a PR comment contains `/CodeSentinel` — resolves PR refs via `gh`, checks out `refs/pull/N/head`, fetches the
  base commit, then runs the review. This is the recommended way to use `/CodeSentinel` without hosting; the
...
- `docs/tag-CodeSentinel.md` rewritten (Actions-first) and README updated.
...
- **The `gpt-5.5` dogfood self-review posted 2 inline `suggest_change` comments** — a clear upgrade over the
  nano model — and caught a genuine security issue: `CodeSentinel-mention.yml` checked out untrusted PR (fork)
  ...
  **Fixed** by gating the job on `author_association` (`OWNER`/`MEMBER`/`COLLABORATOR`) so only trusted
  collaborators can trigger `/CodeSentinel`; applied to the workflow, the docs example, and a security note.
...
`CodeSentinel-mention.yml` — a genuine "pwn request" pattern (a privileged `issue_comment` workflow checks out
...
webhook channel (`@flue/github`, API-based, no checkout) is the safe alternative for live `/CodeSentinel`.
...
### 2026-06-22 — `CodeSentinel init`/`configure`, snyk cleared, target 0.21.0

- **Scaffold command re-added:** `CodeSentinel init` (preferred) writes `.github/workflows/CodeSentinel.yml`;
  `CodeSentinel configure` is a **deprecated alias** that warns it will be removed in the next major version.
  `--force` overwrites an existing file. Implemented in `bin/CodeSentinel.mjs`; README updated.
...
- **Trimmed the published artifact:** `files` → `["dist", "bin"]` (was shipping all of `src/` incl. tests).
  Tarball: **6 files** (down from 38). Verified by clean-installing the tarball and running `CodeSentinel review`
...
- **Decisions for the maintainer:** (1) telemetry was removed — re-add as a flue tool, or drop for good?
  (2) keep a thin `CodeSentinel` CLI bin, or is `flue run` the only entry? (3) optional second deployment mode:
  a `@flue/github` **channel** for live `@CodeSentinel review` comments (webhook server) alongside the one-shot
  CI workflow.
...
- **Verified:** `flue build` succeeds; `node dist/server.mjs` boots and serves the review workflow over
  HTTP. So CodeSentinel publishes as a self-built package and runs locally after build (server), in addition
