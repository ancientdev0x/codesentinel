...
CodeSentinel is a code-review agent built on [flue](https://github.com/withastro/flue), run with
Node + npm. It runs the agent loop on "pi" (flue's built-in tools), reviews PRs in CI via the
GitHub Action and staged changes locally (`npx CodeSentinel review`), and deploys anywhere flue deploys.
...
- Optional `@flue/github` channel: a webhook server mode for live "@CodeSentinel review" comments alongside the one-shot CI workflow.
