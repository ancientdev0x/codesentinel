...
// Uses raw CDP over WebSocket, no Puppeteer/Playwright dependency.
// Requires Node 22+ (built-in WebSocket).
//
// PATCHED for CodeSentinel qa (docs/ambient-qa.md §3a):
//   - endpoint discovery via $CDP_PORT/--port (default 9222) over the DevTools
//     HTTP endpoint — works on Linux/Docker/CI; the upstream macOS
//     DevToolsActivePort-file path did NOT. Or $CDP_WS_ENDPOINT/--ws-endpoint
//     (+ $CDP_HEADERS/--headers) to attach a REMOTE browser (the override seam).
//   - per-PORT daemon sockets + pages cache so parallel drivers on
//     9222/9223/... never collide.
//   - a `fill <target> <selector> <text>` command (focus+select+insertText).
//
...
// Endpoint + isolation are PORT-scoped and resolved at CALL time: extractGlobalFlags
// sets the env before any of these run, and the spawned daemon inherits it.
function port() { return process.env.CDP_PORT || '9222'; }
function sockPrefix() { return `/tmp/cdp-${port()}-`; }
function pagesCache() { return `/tmp/cdp-${port()}-pages.json`; }
function sockPath(targetId) { return `${sockPrefix()}${targetId}.sock`; }

function getHeaders() {
  const raw = process.env.CDP_HEADERS;
  if (!raw) return undefined;
  try { return JSON.parse(raw); } catch { return undefined; }
}

// Resolve the browser-level CDP websocket endpoint.
async function getWsUrl() {
  // Remote override seam: an explicit ws:// endpoint short-circuits discovery.
  if (process.env.CDP_WS_ENDPOINT) return process.env.CDP_WS_ENDPOINT;
  // Local: discover via the DevTools HTTP endpoint on the chosen port.
  const p = port();
  const res = await fetch(`http://127.0.0.1:${p}/json/version`);
  if (!res.ok) throw new Error(`CDP /json/version returned HTTP ${res.status} on 127.0.0.1:${p}`);
  const info = await res.json();
  if (!info.webSocketDebuggerUrl) {
    throw new Error(`No webSocketDebuggerUrl from 127.0.0.1:${p}/json/version`);
  }
...
function listDaemonSockets() {
  ...
  return readdirSync('/tmp')
    ...
    .map(f => ({
      targetId: f.slice(prefix.length, -5),
      socketPath: `/tmp/${f}`,
    }));
}
...
class CDP {
  ...
  async connect(wsUrl, headers) {
    return new Promise((res, rej) => {
      // headers (remote seam): undici's WebSocket accepts an options bag; the
      // local path passes none and uses the standard single-arg form.
      this.#ws = headers ? new WebSocket(wsUrl, { headers }) : new WebSocket(wsUrl);
      this.#ws.onopen = () => res();
...
// Fill a field BY SELECTOR: focus + select existing content, then Input.insertText
// so it REPLACES the value and fires real input events (React/Vue-safe). Raw `type`
// has no selector and inserts at whatever currently has focus — `fill` is what the
// QA recipes use to fill form inputs reliably.
async function fillStr(cdp, sid, selector, text) {
  ...
  `;
  const r = JSON.parse(await evalStr(cdp, sid, focusExpr));
  if (!r.ok) throw new Error(r.error);
  await cdp.send('Input.insertText', { text: text ?? '' }, sid);
  return `Filled ${selector} (${(text ?? '').length} chars)`;
}
...
async function runDaemon(targetId) {
  ...
  const cdp = new CDP();
  try {
    await cdp.connect(await getWsUrl(), getHeaders());
  } catch (e) {
...
const USAGE = `cdp - lightweight Chrome DevTools Protocol CLI (no Puppeteer/Playwright)

Usage: cdp [--port N | --ws-endpoint ws://… [--headers '{"k":"v"}']] <command> [args]

GLOBAL FLAGS (or env)
  --port N            DevTools port to attach to (env CDP_PORT, default 9222)
  --ws-endpoint URL   Attach to a REMOTE browser ws:// endpoint (env CDP_WS_ENDPOINT)
  --headers JSON      Auth headers for the remote endpoint (env CDP_HEADERS)
  ...
  fill    <target> <selector> <text> Focus a field (selects existing) then insertText —
                                    the React/Vue-safe way to fill an input by CSS selector
...
DAEMON IPC (for advanced use / scripting)
  Each tab runs a persistent daemon at Unix socket: /tmp/cdp-<port>-<fullTargetId>.sock
...
const NEEDS_TARGET = new Set([
  ...
  'net','network','click','clickxy','type','fill','loadall','evalraw',
]);

// Pull global flags (--port/--ws-endpoint/--headers) out of argv into env BEFORE
// the `cdp <command> <target>` positional parse, so the target prefix is preserved
// and the spawned daemon (which inherits env) attaches to the same browser.
function extractGlobalFlags(argv) {
  const rest = [];
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (a === '--port') process.env.CDP_PORT = argv[++i];
    else if (a.startsWith('--port=')) process.env.CDP_PORT = a.slice('--port='.length);
    else if (a === '--ws-endpoint') process.env.CDP_WS_ENDPOINT = argv[++i];
    else if (a.startsWith('--ws-endpoint=')) process.env.CDP_WS_ENDPOINT = a.slice('--ws-endpoint='.length);
    else if (a === '--headers') process.env.CDP_HEADERS = argv[++i];
    else if (a.startsWith('--headers=')) process.env.CDP_HEADERS = a.slice('--headers='.length);
    else rest.push(a);
  }
  return rest;
}
...
async function main() {
  ...
  if (cmd === 'list' || cmd === 'ls') {
    ...
    writeFileSync(pagesCache(), JSON.stringify(pages));
  ...
  if (daemonMatches.length > 0) {
    ...
    if (!existsSync(pagesCache())) {
      console.error('No page list cached. Run "cdp list" first.');
      process.exit(1);
    }
    const pages = JSON.parse(readFileSync(pagesCache(), 'utf8'));
  ...
  if (cmd === 'eval') {
    ...
    // fill <selector> <text...> — selector is cmdArgs[0], text is the rest (allows spaces)
    const sel = cmdArgs[0];
    if (!sel) { console.error('Error: selector required'); process.exit(1); }
    const text = cmdArgs.slice(1).join(' ');
    cmdArgs.length = 0;
    cmdArgs.push(sel, text);
  } else if (cmd === 'evalraw') {
