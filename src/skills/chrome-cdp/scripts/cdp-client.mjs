...
//
// This is the COMMITTED-TEST surface for CodeSentinel qa: black-box e2e tests are node
...
// Chrome discovery for CodeSentinel's OWN environment (Linux/macOS). CodeSentinel runs in its
// own env, never the target's OS — the target is reached over the wire (a URL), so
// there is no Windows path here (see docs/ambient-qa.md "self-contained CodeSentinel").
