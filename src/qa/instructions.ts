...
const qaSystemPrompt = (): string =>
  `You are CodeSentinel QA — an autonomous end-to-end QA engineer LEAD. You explore a running product,
...
const qaSystemPromptCli = (): string =>
  `You are CodeSentinel QA — an autonomous end-to-end QA engineer LEAD. The target under test is a NON-WEB,
...
(CodeSentinel can ALSO QA web products by driving a headless Chrome over CDP; for THIS run the target is a CLI,
...
export const buildDriverInstructions = (cfg: QaConfig): string => {
  ...
  return `You are a CodeSentinel QA browser-driver subagent. You are handed exactly ONE catalogued user flow.
...
export const buildCliDriverInstructions = (cfg: QaConfig): string => {
  void cfg // reserved for future per-driver config; keeps the signature stable
  return `You are a CodeSentinel QA cli-driver subagent. You are handed exactly ONE catalogued CLI scenario. The
...
export const buildHealerInstructions = (cfg: QaConfig): string => {
  ...
  return `You are a CodeSentinel QA healer subagent. You are handed exactly ONE broken catalogued flow, its spec,
...
7. OPEN PRs — one per accepted tier:
   ...
   - broken-flow PR — ONE per healed/attempted broken ${v.noun} whose finding was accepted: call
     ...
     title that EMBEDS the slug (e.g. "[CodeSentinel-qa] fix broken ${v.noun}: <slug>"), a body with the diagnosis,
