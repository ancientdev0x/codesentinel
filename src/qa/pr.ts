...
export interface OpenPrArgs {
   ...
   * defaults to a stable per-flow branch (`CodeSentinel-qa/fix/<slug>`).
   */
  branch?: string
  /**
 ...
 *     (`CodeSentinel-qa/fix/<slug>`), PLUS a secondary title-marker search across open
...
const lastPrPath = (workspace: string): string =>
  join(workspace, '.CodeSentinel', 'qa', 'last-pr.json')
