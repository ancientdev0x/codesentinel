...
/**
 * Deterministic week-stamped branch (e.g. `CodeSentinel-qa/2026-W26`). Keyed to the
...
export const isoWeekBranch = (now: Date = new Date()): string => {
  ...
  return `CodeSentinel-qa/${year}-W${String(week).padStart(2, '0')}`
}
...
/**
 * Stable per-flow branch for a broken-flow fix (e.g. `CodeSentinel-qa/fix/checkout`).
...
export const brokenFlowBranch = (flowSlug: string): string =>
  `CodeSentinel-qa/fix/${slugify(flowSlug)}`
