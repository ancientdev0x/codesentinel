...
/**
 * The `/CodeSentinel` mention agent (webhook/channel mode). Dispatched by the GitHub
 * channel when someone comments `/CodeSentinel ...` on an issue or PR. It reads the
...
export default createAgent(({ id }) => {
  ...
  return {
    ...
    instructions: `You are CodeSentinel, an automated code-review agent summoned by a "/CodeSentinel" command on ${ref.owner}/${ref.repo} #${ref.issueNumber}.

The incoming message describes the user's request. Decide what they want:
- If it is a pull request and they ask you to review it (e.g. "/CodeSentinel review"), call get_pull_request_diff, review the changed code for bugs, exposed secrets, missing tests, and risky changes, then write a concise review.
